...
class TrajPredictionEvaluator():
    ...
    def evaluate(self, post_out, data): # post_out是net的输出，data是真值
        traj_pred = post_out['traj_pred']   # [1, 6, 60, 2]，本车6个模态的未来60帧的轨迹 x/y
        prob_pred = post_out['prob_pred']   # [1, 6]，应该只有本车的6个模态的概率
        ...
        if self.config['data_ver'] == 'av1':
            ...
            # for av2
            traj_fut = torch.stack([x['TRAJS_POS_FUT'][0] for x in data["TRAJS"]])  # batch x fut x 2。真值，[1, 60, 2]
        else:
            assert False, 'Unknown data_ver: {}'.format(self.config['data_ver'])

        # to np.ndarray
        traj_pred = np.asarray(traj_pred.cpu().detach().numpy()[:, :, :, :2], np.float32)   # net预测值：[1, 6, 60, 2]]
        prob_pred = np.asarray(prob_pred.cpu().detach().numpy(), np.float32)    # net预测值：[1, 6]
        traj_fut = np.asarray(traj_fut.numpy(), np.float32) # 真值：[1, 60, 2]
